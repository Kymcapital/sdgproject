...
<div class="row">
    <div class="col-md-12">
        <form id="championForm" name="championForm" action="{{ route('responses.filter') }}" method="post">
            @csrf
            <div class="input-group sdginfocus align-items-end">
                ...
                <button class="btn btn-primary" id="btn-save" type="submit">FILTER</button>
                <a class="btn btn-primary ml-4" href="{{ route('sdg_response.export') }}">Export Bulk Data</a>
            </div>
        </form>
    </div>
</div>
...
<div class="row justify-content-center">
    <div class="col-md-12">
        ...
        <form id="newSubmissionForm" name="newSubmissionForm" action="{{ route('responses.store') }}" method="post" enctype="multipart/form-data">
            @csrf
                                        ...
                                        <tr class="{{$highlight}}">
                                                ...
                                                @if(in_array(Session::get('userData')->permission_id, [1]) && in_array(Session::get('userData')->roles_name, ['Super Admin','Admin']))
                                                    <td>   
                                                        ...
                                                        @else
                                                            ...
                                                            <input type="number" name="" value="{{ old('sub_total') ?? $kpi->sub_total ?? 0 }}" min="0" disabled class="w-100 py-0" min="0" class="w-100 py-0"><!-- Add New Submission -->
                                                        @endif
                                            ...
                                            <td>
                                                <span>
                                                    @if($kpi->sub_total != 0)
                                                            @if($kpi->status)
                                                                <a href="{{ route('responses.status', ['id' => $kpi->id, 'status' => 0]) }}" class="text text-danger"><i class="fas fa-lock-open"></i> Not Approved</a>
                                                            @else
                                                                <a href="{{ route('responses.status', ['id' => $kpi->id, 'status' => 1]) }}" class="text text-success"><i class="fas fa-lock"></i> Approved</a>
                                                            @endif
                                                        @else
